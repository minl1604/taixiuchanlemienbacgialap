...
function HomePageContent() {
  ...
  const triggerConfetti = useCallback(() => {
    ...
    const count = Math.floor(Math.random() * 21) + 30; // 30 to 50 pieces
  ...
  const memoizedConfetti = useMemo(() => confettiPieces, [confettiPieces]);
  const handleSpin = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        ...
        const resultMessage = `Kết quả kỳ ${newRound.roundNumber}: ${newRound.taiXiu} - ${newRound.chanLe}`;
        setAriaLiveMessage(resultMessage);
        if (profit !== null) {
          if (profit > 0) {
            toast.success(`Kỳ #${newRound.roundNumber} - Thắng!`, { description: `L��i nhuận: +${profit.toLocaleString('vi-VN')} VND` });
            triggerConfetti();
          } else if (profit < 0) {
    ...
    }, 500); // Artificial delay for spin animation feel
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans relative overflow-x-hidden flex flex-col" onClick={handleUserInteraction} onTouchStart={handleUserInteraction}>
      ...
      <AlertDialog open={showDisclaimer} onOpenChange={setShowDisclaimer}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>L��u ý quan trọng</AlertDialogTitle>
            <AlertDialogDescription>
              Đây là một ���ng dụng giả lập chỉ dành cho mục đích giải trí. Mọi kết quả đều là ngẫu nhiên và không liên quan đến kết quả xổ số thực tế. Ứng dụng này không sử dụng tiền thật và không dành cho mục đích cờ bạc. Vui lòng chơi có trách nhiệm.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="fixed inset-0 pointer-events-none z-50">{memoizedConfetti}</div>
